...
const REFERRAL_BONUS = 1000; // Points referrer gets
const REFEREE_BONUS = 500;   // Points referee gets

export function ReferralCard() {
  const { profile, referralStats, isLoading } = useProfile();
  ...
  const referralCount = referralStats?.count || profile?.total_referrals || 0;
  const pointsEarned = referralStats?.earned || (referralCount * REFERRAL_BONUS);
    ...
    <div className="space-y-4">
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between text-sm">
            ...
            <span className="font-semibold flex items-center gap-1">
              ...
              +{REFERRAL_BONUS.toLocaleString()} points
            </span>
          </div>
          <div className="flex items-center justify-between text-sm">
            ...
            <span className="font-semibold flex items-center gap-1">
              ...
              +{REFEREE_BONUS.toLocaleString()} bonus
            </span>
          </div>
        </div>
      </Card>
      ...
      <Card className="p-4 bg-muted/30">
        ...
        <ul className="text-sm text-muted-foreground space-y-1">
          ...
          <li>• Share your personal referral link above</li>
        </ul>
      </Card>
    </div>
  );
