...
import { 
  ...
  Sparkles, Shield, Zap, LogOut, User, Loader2
} from 'lucide-react';
...
export default function Profile() {
  const { profile, leaderboardEntry, totalMultiplier, referralStats, isLoading, isAuthenticated } = useProfile();
  const { user, isTelegram } = useTelegram();
  const { badgeCount, userBadges, badgesByCategory, isLoading: badgesLoading } = useBadges();
          ...
          <Card className="p-6 bg-gradient-to-br from-primary/10 via-accent/5 to-background border-primary/20">
            <div className="flex items-start gap-4">
              <Avatar className="w-20 h-20 border-2 border-primary/30">
                <AvatarImage src={user?.photo_url || profile?.avatar_url} />
